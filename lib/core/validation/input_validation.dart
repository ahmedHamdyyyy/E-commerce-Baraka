...
class InputValidation {
  ...
  static String? email(value) {
    if (value == null) return 'must not be empty';
    if (!_emailReg.hasMatch(value)) return 'not valid';
    return null;
  }

  static String? passwordValidation(value) {
    if (value == null) return 'must not be empty';
    if (value.length <= 7) return 'too weak';
    if (value.length >= 51) return 'too Strong';
    return null;
  }
}
*/
